...
module.exports = HandleMsg = async (aruga, message) => {
	...
	// Filter Banned People
      ...
      case 'bokepindo':
            await aruga.sendText(from, menuId.textbokepindo())
            break
	...
	case 'brainly':
        ...
        case 'nulis':
            ...
            await aruga.sendImage(from, `${nulisp}`, '', 'bot nulis by ubed ganteng...', id)
            .catch(() => {
                aruga.reply(from, 'Ada yang Error!', id)
            })
            break
	...
	if (!isBotGroupAdmins) return aruga.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        // Random Kata
      	case 'motivasi':
            ...
            break
	case 'katakotor':
            fetch('https://raw.githubusercontent.com/ubed354311/katakotor/main/katakotor.txt')
            .then(res => res.text())
            .then(body => {
                let splitmotivasi = body.split('\n')
                let randommotivasi = splitmotivasi[Math.floor(Math.random() * splitmotivasi.length)]
        ...
        case 'anime':
            ...
            if (args[0] == 'random' || args[0] == 'waifu' || args[0] == 'husbu' || args[0] == 'neko') {
                ...
                .then(body => {
                    ...
                    aruga.sendFileFromUrl(from, randomnimex, '', 'dasar wibu bau bawang..', id)
                })
                .catch(() => {
                    aruga.reply(from, 'Ada yang Error!', id)
                })
        ...
        case 'kpop':
            ...
            if (args[0] == 'blackpink' || args[0] == 'exo' || args[0] == 'bts') {
                ...
                .then(body => {
                    ...
                    aruga.sendFileFromUrl(from, randomkpopx, '', 'dasar kpopers anjing memek..', id)
                })
                .catch(() => {
                    aruga.reply(from, 'Ada yang Error!', id)
                })
